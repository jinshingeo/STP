"""
Thermal-Inclusive PPA 분석 — v2 (S-DoT 실측 기반)
=====================================================
버전 히스토리:
  v1_sim  (2026-03-18): 링크 유형(bridge/major/local) 기반 시나리오 하드코딩
  v2_sdot (2026-04-01): S-DoT 57개 센서 실측 UTCI → IDW 보간 → 링크별 속도계수

변경점 (v1 → v2):
  - SCENARIOS 딕셔너리(고정값) 제거
  - link_utci_by_hour.csv 에서 링크별 speed_factor 로드
  - 분석 시간대: 7시 / 10시 / 13시 (기존 A/B/C 시나리오와 동일 시각)
  - 교량 패널티는 IDW 보간 단계에서 이미 반영됨 (speed_factor × 0.7)

한계 (추후 개선 예정):
  - MRT = tdb + 3.5 × UV_index (1차 근사, SVF/그늘 미반영)
  - 풍속 1.5 m/s 가정 (ASOS 실측값으로 교체 예정)
  - IDW 보간: 센서 간 미기후 차이 미세하게 반영 못함
"""
...
LINK_UTCI    = os.path.join(BASE, "link_utci_by_hour.csv")
OUT_DIR      = BASE
FIG_DIR      = os.path.join(BASE, "figures")
os.makedirs(FIG_DIR, exist_ok=True)

VERSION      = "v2_sdot"
WALK_SPEED   = 4.5 * 1000 / 3600   # 기본 보행속도 m/s (4.5 km/h)
TIME_BUDGET  = 30 * 60              # 시간 예산 (초)
EUNGBONG_LAT, EUNGBONG_LON = 37.5428, 127.0357

# 분석할 시간대 (v1 시나리오 A/B/C에 대응)
TARGET_HOURS = {
...
for ax, (hour, res) in zip(axes, results.items()):
    ...
    if 'name' in edges_gdf.columns:
        bridge_e = edges_gdf[edges_gdf.get('bridge', pd.Series()) == 'yes'] if 'bridge' in edges_gdf.columns else pd.DataFrame()
        if len(bridge_e) > 0:
    ...
    ax.set_title(
        f"{res['label']}\n"
        f"도달 가능: {res['reachable_count']:,}개 ({res['reachable_pct']}%) | Classic 대비 -{decline}%",
        fontsize=10
    )
    ax.set_axis_off()

plt.suptitle(
    f"Thermal PPA {VERSION} — S-DoT 실측 기반\n"
    "출발지: 응봉역 | 시간예산: 30분 | S-DoT IDW 보간 속도계수 적용",
...
summary = {
    "_version": VERSION,
    "_date": "2026-04-01",
    "_data_source": "S-DoT 2025.07.28-08.03, IDW 보간, v=1.5m/s 가정",
    "_limitations": "MRT=tdb+3.5*UV(1차근사), 풍속가정, SVF미반영",
...
for hour, res in results.items():
    ...
    summary[key] = {
        ...
        "decline_from_classic_pct": round(
            (classic['reachable_nodes'] - res['reachable_count'])
            / classic['reachable_nodes'] * 100, 1
        )
    }
...
for hour, res in results.items():
    decline = round((classic['reachable_nodes'] - res['reachable_count']) / classic['reachable_nodes'] * 100, 1)
    print(f"  {res['label']}: {res['reachable_count']:,}노드 ({res['reachable_pct']}%) → -{decline}%")
print(f"\n결과 저장: {out_json}")
