"""
응봉동 vs 성수동 Thermal PPA 비교 시각화 — v2_sdot
=======================================================
출력:
  figures/comparison_ppa_maps.png   — 2×3 지도 (두 지역 × 3 시간대)
  figures/comparison_ppa_chart.png  — PPA 감소율 비교 차트
"""
...
fig.suptitle(
    "Thermal PPA 비교: 응봉동(열 취약) vs 성수동(비교군) — v2_sdot\n"
    "데이터: S-DoT 2025.07.28~08.03 | 시간예산 30분 | 밝은색 = 열로 인해 상실된 접근 공간",
    fontsize=13, fontweight='bold', y=1.01
)
plt.tight_layout(rect=[0, 0.04, 1, 1])
out_map = os.path.join(FIG_DIR, 'comparison_ppa_maps.png')
...
fig.suptitle(
    '응봉동 vs 성수동 PPA 비교 — v2_sdot (S-DoT 실측 기반)\n'
    '출발지: 각 지역 대표 집계구 | 시간예산 30분 | 목적지: 성동구민종합체육센터',
    fontsize=12, fontweight='bold', y=1.02
)
plt.tight_layout()
out_chart = os.path.join(FIG_DIR, 'comparison_ppa_chart.png')
